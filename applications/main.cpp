...
// these are sufficient to reproduce the run without guessing CLI overrides.
static void snapshotRunInputs(int argc, char** argv,
                              const std::string& data_file_path,
                              const std::string& out_dir,
                              const std::string& out_name)
{
    const std::string params_dst = out_dir + "/" + out_name + "_params.data";
    std::ifstream src(data_file_path, std::ios::binary);
    std::ofstream dst(params_dst, std::ios::binary);
    if (src && dst) {
        dst << src.rdbuf();
    } else {
        std::cerr << "Warning: could not snapshot params to " << params_dst << std::endl;
    }

    const std::string invocation_dst = out_dir + "/" + out_name + "_invocation.sh";
    std::ofstream inv(invocation_dst);
    if (inv) {
        inv << "#!/usr/bin/env bash\n";
        inv << "# Auto-generated by meshtools3d. Re-run this to reproduce the outputs.\n";
        ...
        std::cerr << "Warning: could not write invocation log to " << invocation_dst << std::endl;
    }
}
...
int main(int argc,char **argv)
{
  ...
  snapshotRunInputs(argc, argv, data_file_name, out_dir, out_name);
